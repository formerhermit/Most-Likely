...
const PHASE_CARDS = {
  ...
  afterWork: {
    ...
    body: [
      ...
      'Of course, a real model looks nothing like this. There isn’t a table of words, or a conveyor belt.',
      'But the same two things made it. It read a bunch of stuff that it was given, and then people marked its answers until it learned what they liked.'
    ]
  }
};
...
/* ---- Act 1 verdict copy ----
   The one-line verdict under each document's bar (curveVerdict in
   js/pretrain.js picks the branch). Each branch is a small pool rather than
   one fixed string: the act runs ten documents and the flat "no change"
   branch alone can come up six times — a line that good, repeated that
   often, stops reading as a voice and starts reading as a bug. Pools also
   give a second playthrough something the first didn't have.

   Drawn through `drawFrom` (js/state.js), so a repeat is never back to
   back. */
const VERDICTS = {
  first: [
    'Baby’s first book!',
